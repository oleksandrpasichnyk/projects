// import {startGame, playRound, resetGame, isStarted} from './game.js';

const gameActionsButtons = document.querySelector('.game-actions-buttons');
...
gameActionsButtons.addEventListener('click', function(event){
    let role = event.target.closest('button').dataset.role;
    if(!isStarted) {
        startGame();
    }
    playRound(role);
